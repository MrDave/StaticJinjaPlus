...
Пример кода c вызовом `extends` из `index.html file`
...
- Вызов `include` позволяет добавить один шаблон в другой шаблон. Название подключаемого шаблона передается фильтру в качестве параметра.
